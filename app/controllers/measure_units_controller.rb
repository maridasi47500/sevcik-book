class MeasureUnitsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @measure_unit.update(measure_unit_params)
        format.html { redirect_to @measure_unit, notice: "Measure unit was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to measure_units_path, notice: "Measure unit was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_measure_unit
      @measure_unit = MeasureUnit.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def measure_unit_params
      params.expect(measure_unit: [ :name ])
    end
end
